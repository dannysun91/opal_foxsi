...
sim_angles     = np.array([])#np.concatenate([np.array([0,1.2,1.3,1.4]),np.arange(2,4.0,.1)])#np.arange(0,2,.2)#np.arange(0,1.2,.2)#
...
for ai in range(0,len(sim_angles)):
    ...
    print(sdirf)
    sdirfile        = os.path.join(sdirf,sfile+'.png')   # Sim File Directory 
    ...
    sdirnfile       = os.path.join(sdirf,"Image_Processing",snewfile+'.png') # New File Directory
    ...
    if plt_flg:
        ...
        sy_off =  (sboxlen[-1]/2.0)-scenters[0,1]*ssf[1] ####DANNY#####
